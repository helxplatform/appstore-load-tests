...
function setXCSRF(requestParams, context, ee, next) {
    ...
    if (!csrfCookie) {
        console.error("CSRF token not found in cookies. Available cookies:", cookies.map(c => c.key));
        return next(new Error("CSRF token not found - there may be a serious problem with the request flow"));
    }
